...
import math
# import matplotlib.pyplot as plt # i ditched this because on low hardware it can cause issues and wandb is better anyway
...
class BTRGodotEnv(CleanRLGodotEnv):
    def __init__(self, stack_size=4, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.stack_size = stack_size
        # The stack for all parallel envs: (N, 4, 84, 84)
        self.stack = np.zeros((self.n_parallel, stack_size, 84, 84), dtype=np.float32)

        # Action space is handled the same way as PPO: a single discrete branch
        # sent as shape (num_envs, 1) — no action_map needed, Godot handles the mapping

    def _format_info(self, info_list):
        """Converts List[Dict] from Godot to Dict[List] for the main loop."""
        # print(info_list)
        formatted = {
            "Ignore": [],
            "First": [],
            "final_observation": [],
            "lifes_left_and_aliens_shot": [],
            "busy_lost_won": [],
            "in_game_score": [],
            "experiment_rewards": [],
        }
        for i in range(self.n_parallel):
            env_info = info_list[i] if i < len(info_list) else {}

            formatted["Ignore"].append(env_info.get("Ignore", False))
            formatted["First"].append(env_info.get("First", False))
            formatted["lifes_left_and_aliens_shot"].append(env_info.get("lifes_left_and_aliens_shot", False))
            formatted["busy_lost_won"].append(env_info.get("busy_lost_won", "busy"))
            formatted["in_game_score"].append(env_info.get("in_game_score", 0))
            formatted["experiment_rewards"].append(env_info.get("experiment_rewards", None))

            # Handle final observation for truncated steps
            # We process it the same way as a normal observation
            f_obs_raw = env_info.get("final_observation", None)
            if f_obs_raw is not None:
                # Decode if hex, otherwise just use as array
                f_obs = hex_to_ndarray(f_obs_raw, shape=(84, 84, 1)).squeeze(-1)
            else:
                f_obs = None
            formatted["final_observation"].append(f_obs)
        return formatted

    def _process_obs(self, obs):
        """Decodes hex strings and reshapes for the stack."""
        decoded_batch = []
        for i in range(self.n_parallel):
            # Decode hex -> (84, 84, 1) -> squeeze to (84, 84)
            frame = hex_to_ndarray(obs[i], shape=(84, 84, 1))
            decoded_batch.append(frame.squeeze(-1))
        return np.array(decoded_batch)  # Returns (N, 84, 84)

    def reset(self, seed=0):
        obs, info_list = super().reset(seed=seed)
        processed_obs = self._process_obs(obs)

        # Fill stack with the first frame (N, 4, 84, 84)
        for i in range(self.stack_size):
            self.stack[:, i, :, :] = processed_obs

        return self.stack.copy(), self._format_info(info_list)

    def step(self, actions):
        # Mirror PPO: actions arrive as (num_envs, 1) shaped array, pass straight to Godot
        next_obs_raw, reward, terminated, truncated, info_list = super().step(actions)

        # Update Stack
        new_frames = self._process_obs(next_obs_raw)
        self.stack = np.roll(self.stack, shift=-1, axis=1)
        self.stack[:, -1, :, :] = new_frames

        # Handle auto-reset zeros in stack
        for i in range(self.n_parallel):
            if terminated[i] or truncated[i]:
                self.stack[i, :-1, :, :] = 0
        return self.stack.copy(), np.array(reward), terminated, truncated, self._format_info(info_list)
    ...
    def set_curriculum(self, level: int, nums: list, strings: list):
        ...
        # We iterate through the sub-environments (GodotEnv instances)
        for env in self.envs:
            # These are the correct internal methods for the godot_rl library
...
class TrainingMonitor:
    ...
    def update(self, global_step):
        ...
        if gpu:
            ...
            print(
                f"VRAM:  {f'{curr_vram:.2f}GB / {self.gpu_total_mem:.1f}GB':<{col_1}} | {f'{vram_pct:.0f}%':^{col_2}} | Peak: {self.max_vram:.2f}GB")
        else:
            print(f"GPU:   {'No GPU detected':<{col_1}}")

        print(
            f"RAM:   {f'{curr_ram_gb:.1f}GB / {self.ram_total_gb:.1f}GB':<{col_1}} | {f'{ram.percent:.0f}%':^{col_2}} | Peak: {self.max_ram:.1f}GB")
        print(
            f"CPU:   {f'{curr_cpu_pct:.1f}% @ {cpu_freq:.1f}GHz':<{col_1}} | {f'--':^{col_2}} | Peak: {self.max_cpu:.1f}%")
...
originally this was the setup to play Mario Kart Wii but i adopted it for GODOT
"""
...
class PER:
    def __init__(self, size, device, n, envs, gamma, alpha=0.2, beta=0.4, framestack=4, imagex=84, imagey=84,
                 rgb=False):
    ...
    def append_pointer(self, stream, prio):
        ...
        while len(self.state_buffer[stream]) >= self.framestack + self.n_step and len(
            ...
            self.pointer_mem[self.point_mem_idx] = (np.array(state_array, dtype=int),
    ...
    def sample(self, batch_size, count=0):
        ...
        weights = (
                              self.capacity * probs) ** -self.alpha  # self.beta originally this was an accident but actually performed better
...
class Agent:
    def __init__(self, n_actions, input_dims, device, num_envs, agent_name, total_frames, testing=False, batch_size=256
                 ...
                 imagey=84, per_alpha=0.2, max_mem_size=960_000, eps_steps=2_000_000, eps_disable=True, n=3,
        ...
        self.total_grad_steps = (self.total_frames - self.min_sampling_size) / (
                    (self.replay_period * self.num_envs) / self.spi)
    ...
    def save_checkpoint(self, path, steps, wandb_run_id=None):
        """Saves full resumable checkpoint: model + optimizer + training state."""
        torch.save({
            "steps": steps,
        ...
        print(f"[CHECKPOINT] Saved to '{path}' at step {steps:_}")

    def load_checkpoint(self, path, device):
        """Loads model + optimizer state from a checkpoint. Returns saved steps and wandb_run_id."""
        ckpt = torch.load(path, map_location=device)
        self.net.load_state_dict(ckpt["model_state_dict"])
        self.tgt_net.load_state_dict(ckpt["model_state_dict"])  # keep target net in sync
        self.optimizer.load_state_dict(ckpt["optimizer_state_dict"])
        steps = ckpt.get("steps", 0)
        wandb_run_id = ckpt.get("wandb_run_id", None)
        # replay buffer is not saved — env_steps reset to 0 so buffer refills before learning resumes
        # this causes a ~200k step re-exploration phase on resume, which is an inherent BTR limitation
        self.env_steps = 0
        # restore grad_steps for correct target network sync and logging
        self.grad_steps = ckpt.get("grad_steps", 0)
        # set epsilon to near-zero so agent exploits learned policy instead of exploring randomly
        self.loading_checkpoint = True
        self.epsilon.eps = self.epsilon.eps_final
        print(f"[CHECKPOINT] Loaded from '{path}'")
        print(f"[CHECKPOINT] Resuming at step {steps:_}, wandb_run_id={wandb_run_id}")
        return steps, wandb_run_id
...
def non_default_args(args, parser):
    result = []
    # args to exclude from the agent name (infrastructure args, not algorithm hyperparams)
    exclude = {"env_path", "checkpoint", "track", "wandb_project_name", "seed",
               "torch_deterministic", "include_evals", "eval_envs", "num_eval_episodes",
               "analy", "num_envs", "frames", "repeat", "device", "testing",
               "framestack", "exp_name"}
    for arg in vars(args):
        ...
        if user_val != default_val and default_val != "NameThisGame":
...
def main():
    # --- BATCH 4: curriculum variables (mirrors PPO exactly) ---
    include_curriculum_learning = False
    curriculum_lvl2_at_step = 40_000_000  # moving_avg_gamescore would probably be a better trigger
    ...
    # game is hardcoded — not a CLI arg so it doesn't pollute the wandb config
    game = "SpaceInvadersG"
    parser.add_argument("--env_path", type=str, default=None)
    parser.add_argument('--num_envs', type=int, default=1)
    parser.add_argument('--frames', type=int, default=2000000000)
    parser.add_argument('--eval_envs', type=int, default=2)
    ...
    parser.add_argument('--eps_steps', type=int, default=20_000_00)
    parser.add_argument('--eps_disable', type=int, default=1)

    # --- BATCH 1: tracking + reproducibility ---
    ...
    monitor = TrainingMonitor(num_envs=envs)
    monitor.update(global_step=0)

    # --- BATCH 1: run identity, seeds, wandb init ---
    ...
    # --- BATCH 3b: peek at checkpoint for wandb run id before init ---
    resumed_wandb_run_id = None
    if args.checkpoint:
        args.checkpoint = str(Path(args.checkpoint).resolve())  # resolve before os.chdir() changes cwd
        _ckpt_peek = torch.load(args.checkpoint, map_location="cpu")
        resumed_wandb_run_id = _ckpt_peek.get("wandb_run_id", None)
        del _ckpt_peek
        print(f"[CHECKPOINT] Peeked at checkpoint: wandb_run_id={resumed_wandb_run_id}")
    ...
    if testing:
        ...
        num_envs = envs
        n_steps = frames
        eval_every = 250_000  # <<<<<<<<<<<<<<<<<-------------------------- qucik debugging | original: eval_every = 250_000
    ...
    # env = DolphinEnv(envs)
    # The arguments here go to CleanRLGodotEnv via super()
    env = BTRGodotEnv(stack_size=4, env_path=args.env_path, show_window=True,
                      n_parallel=envs)  # speedup=10 -> using this in here overwrites the speedup thats set in the sync node

    print(
        env.single_observation_space)  # AttributeError: 'CleanRLGodotEnv' object has no attribute 'observation_space'. Did you mean: 'single_observation_space'?
    print(env.single_action_space)  # AttributeError: 'CleanRLGodotEnv' object has no attribute 'action_space'

    # oiginal n_actions=env.action_space[0].n
    agent = Agent(n_actions=env.single_action_space, input_dims=[framestack, 84, 84], device=device, num_envs=num_envs,
                  ...
                  munch_alpha=munch_alpha, grad_clip=grad_clip, imagex=84, imagey=84, spi=spi)
    ...
    scores = []
    window = 200  # moving average window for the eval block
    observation, info = env.reset()
    processes = []

    # --- BATCH 3b: load checkpoint if provided, else start fresh ---
    if args.checkpoint:
        print(f"Resuming from checkpoint: {args.checkpoint}")
        steps, _ = agent.load_checkpoint(args.checkpoint, device)
        next_eval = ((steps // eval_every) + 1) * eval_every
        print(f"Resumed at steps={steps:_}, next eval at {next_eval:_}")
        if args.track:
            append_resume_step(steps)  # log the step we're resuming from, mirrors PPO
    else:
        steps = 0
        next_eval = eval_every

    # --- BATCH 2: PPO-aligned tracking variables ---
    ...
    # --- BATCH 3a: wrap training loop for safe interrupt handling ---
    try:
        while steps < n_steps:
            steps += num_envs
            try:
            ...
            # Godot RL Agents is kind of build for synced envs - but maybe async would boost performance a little?
            # probably depends on the FPS https://gemini.google.com/share/99a01df49587 so i will test it with synced first
            # env.step_async(action)
            # agent.learn()
            # observation_, reward, done_, trun_, info = env.step_wait()

            observation_, reward, done_, trun_, info = env.step(
            ...
            if args.track and experiment_rewards is None:
                for er in info["experiment_rewards"]:
                    if er is not None:
                        experiment_rewards = er
                        wandb.config.update({"#experiment_rewards": str(experiment_rewards)})
                        break

            try:  # note: BTR info is a dict-of-lists from _format_info(), not a list of dicts like PPO
                for stream in range(num_envs):
                    if info["busy_lost_won"][stream] in ["lost", "won"]:
                        won_lost_list.append(busy_lost_won_map[info["busy_lost_won"][stream]])
                        ingame_score_list.append(info["in_game_score"][stream])
            except Exception:
            ...
            for i in range(num_envs):
                ...
                if done_[i] or trun_[i]:
                    ...
                    if args.track:
                        # TODO: if several envs end their episodes at the same step then wandb receives several y-values for 1 x-value (the current step) but its not a real problem and unlikely
                        wandb.log({
            ...
            for stream in range(num_envs):
                # 1. Skip if the 'Ignore' flag is set (handled as a list of bools by the wrapper)
                if info["Ignore"][stream]:
                    continue

                # 2. Reset the starting observation if it's the first frame of an episode
                if info["First"][stream]:
                    observation[stream] = observation_[stream]

                # 3. Handle Truncation: Use final_observation if it exists, else use current frame
                if trun_[stream] and info["final_observation"][stream] is not None:
                    next_obs = info["final_observation"][stream]
                else:
                    next_obs = observation_[stream]

                # 4. Store the transition
                agent.store_transition(observation[stream], action[stream], reward[stream],
                                       next_obs, done_[stream], trun_[stream], stream=stream)

            # Advance the state
            observation = observation_

            # --- BATCH 2: PPO-aligned print + monitor interval ---
            # TODO: change 1_000 back to 42_000 for real training (42_000 mirrors PPO)
            if steps % 42_000 == 0 and len(scores) > 0:
                ...
                print(
                    f"BTR avg score {avg_score:.2f} | "
                    f"avg ingame points {avg_ingame_score:.0f} | "
                    f"games={episodes} | "
                    f"total_timesteps {steps:_} | "
                    f"Win/Loss ratio {moving_win_loss_ratio:.2f} | "
                    f"fps {(steps - last_steps) / (time.time() - last_time):.2f}",
                    flush=True
                )
                last_steps = steps
                last_time = time.time()

                monitor.update(global_step=steps)

            # --- BATCH 4: curriculum learning gate (mirrors PPO exactly) ---
            if include_curriculum_learning:
                if steps >= curriculum_lvl2_at_step and curriculum_lvl2_sent_counter <= 10:
            ...
            # --- BATCH 3a: periodic checkpoint save every 250k steps (only when tracking) ---
            if args.track and steps % 250_000 < num_envs:
                ckpt_path = "checkpoint.pth"
                agent.save_checkpoint(ckpt_path, steps, wandb_run_id=wandb.run.id)
                print(f"[CHECKPOINT] Saved at step {steps:_}")
        ...
        # --- BATCH 3a: clean completion save (inside try, after while loop exits normally) ---
        if args.track:
            ckpt_path = "checkpoint.pth"
            agent.save_checkpoint(ckpt_path, steps, wandb_run_id=wandb.run.id)
            wandb.save(ckpt_path, policy="now")
            print("[CHECKPOINT] Training complete. Final checkpoint saved and uploaded to wandb.")
        else:
        ...
        if args.track:
            print("\nInterrupted! Saving and uploading checkpoint...")
            agent.save_checkpoint("checkpoint.pth", steps, wandb_run_id=wandb.run.id)
            wandb.save("checkpoint.pth", policy="now")
            print(f"[CHECKPOINT] Checkpoint saved and uploaded to wandb at step {steps:_}.")
        else:
